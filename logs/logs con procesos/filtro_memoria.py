-------------------filtro de logs en memoria-----------------archivo filtro_memoria.py
import os
import sys
import re
...
def extraer_bloques_log(input_path: str) -> list:
    bloques = []
...
if __name__ == "__main__":
    ...
    try:
        ...
        bloques = extraer_bloques_log(input_original)

        reporte = generar_reporte_logs(bloques)

        total_logs = len(bloques)
        
        print("✅ Proceso completo en memoria:")
        print(f"- Bloques de log procesados: {total_logs}")
        print(f"- Errores encontrados: {sum(1 for k in reporte if k[0] == 'ERROR')}")
