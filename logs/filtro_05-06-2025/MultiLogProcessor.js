...
const MultiLogProcessor = () => {
  ...
  const eliminarServidor = async (idServidor) => {
    // 1. Detener proceso si está activo
    setEliminando(idServidor);
    try {
    ...
    if (window.confirm("¿Está seguro de eliminar este servidor? Se perderán todos los procesos y errores asociados permanentemente.")) {
      try {
        ...
        // Limpiar configuración persistente
        const config = JSON.parse(localStorage.getItem('servidoresConfig')) || {};
        delete config[idServidor];
        localStorage.setItem('servidoresConfig', JSON.stringify(config));
