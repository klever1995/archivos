...
def consultar_procesos(
    ...
    tipo_proceso: Optional[str] = Query(None, description="Tipo de proceso (LOCAL o FILTRADOREMOTO)"),
    limite: int = Query(100, description="Límite de registros", ge=1, le=1000)
):
    try:
        # Contexto de Flask (sin cambios)
        ...
        with flask_app.app_context():
            ...
            if tipo_proceso:
                query = query.filter(LoProcesos.tipoProceso == tipo_proceso.upper())
            ...
            return {
                ...
                "resultados": [{
                    ...
                    "archivo": p.archivo,
                    "estado": p.estado,
                    "tipo_proceso": p.tipoProceso,  # Nuevo campo incluido
