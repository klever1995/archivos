    ...
    <div className="tablero-container">
      ...
      <div className="servidores-grid">
        {servidores.map((servidor) => {
          ...
          const ordenNiveles = ['ERROR', 'WARN', 'INFO', 'DEBUG'];
          const logsPorNivel = datosServidor.logs_por_nivel || {};

          const niveles = ordenNiveles.filter((nivel) => logsPorNivel[nivel] !== undefined);
          const valores = niveles.map((nivel) => logsPorNivel[nivel]);
          const colores = ['#FF6384', '#FFCE56', '#36A2EB', '#4BC0C0'];
          ...
          const datosNiveles = {
            ...
            datasets: [
              {
                data: valores,
                backgroundColor: colores.slice(0, niveles.length),
                borderWidth: 1,
              },
            ],
          };
