...
def obtener_logs_filtrados(
    id_servidor: int = Path(..., description="ID del servidor a consultar"),
    idEmpresa: int = Query(..., description="ID de la empresa"),
    nivel: str = Query(..., description="Nivel de log a filtrar (ERROR, WARN, INFO)"),
    dias_atras: int = Query(7, description="Rango de días para filtrar"),
    limite: int = Query(100, description="Límite de registros", ge=1, le=1000),
    ...
    try:
        with flask_app.app_context():
            ...
            if nivel not in ['ERROR', 'WARN', 'INFO']:
                raise HTTPException(
                    status_code=400,
                    detail="Nivel debe ser ERROR, WARN o INFO"
                )
            ...
            return {
                ...
                "resultados": [{
                    ...
                    "lineas": log.lineas  # Nuevo campo añadido
                } for log in logs]
            }
