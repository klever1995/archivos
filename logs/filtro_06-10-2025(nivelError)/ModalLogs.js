...
const ModalLogs = ({ nivel, logs, onClose, servidorNombre, fechaDesde, fechaHasta, cargando }) => {
  ...
  // Sincroniza filtros de fecha con props del componente padre
  useEffect(() => {
    setFiltros(prev => ({
      ...prev,
      desdeFecha: fechaDesde || new Date().toISOString().split('T')[0],
  ...
  // Maneja estados de carga y validación de datos
  useEffect(() => {
                ...
                logsOrdenados.map((log, index) => (
                  <React.Fragment key={log.id_log || index}>
                    <tr className={`fila-log ${log.nivel.toLowerCase()}`}>
                      ...
                      <td className="nivel-error-celda">
                        <span className={`badge-nivel ${log.nivelError?.toLowerCase() || 'sin-nivel'}`}>
                          {log.nivelError || 'Sin nivel'}
                        </span>
                      </td>
