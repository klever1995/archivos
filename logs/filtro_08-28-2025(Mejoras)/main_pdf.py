...
async def process_query(request: QueryRequest):
    ...
    try:
        ...
        # 0. Obtener texto completo para usarlo en fallbacks
        ...
        if route == "rag":
            ...
            if not response or "No lo sé" in response or "no encuentro" in response.lower():
                ...
                # ✅ CORREGIDO: Pasar contexto completo al CAG
                response = cag_engine.process_query(request.query, external_context=full_text)
                source = "full_context"
                route = "cag"
        else:
            # ✅ CORREGIDO: Pasar contexto completo al CAG
