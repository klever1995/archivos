...
def eliminar_servidor(id_servidor: int = Path(..., description="ID del servidor a eliminar")):
    with flask_app.app_context():
        try:
            ...
            # --- PASO 1: Eliminar datos dependientes (solo para remotos) ---
            if servidor.esRemoto:
                # A. Borrar logs remotos crudos
                db.session.query(loLogsremotos)\
                    .filter_by(idServidor=id_servidor)\
                    .delete()

                # B. Borrar interpretaciones remotas
                db.session.query(loInterpretacionremota)\
                    .filter_by(idServidor=id_servidor)\
                    .delete()

                # C. Borrar logs interpretados vinculados al servidor
                db.session.query(loLogs)\
                    .filter_by(idServidor=id_servidor)\
                    .delete()

                # D. Borrar procesos de filtrado/interpretación
                db.session.query(LoProcesos)\
                    .filter_by(idServidor=id_servidor)\
                    .delete()

                # (Opcional) Validar si hay otros servidores usando el mismo acceso
