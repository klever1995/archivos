...
def insertar_logs_a_bd(reporte):
    ...
    for (nivel, categoria, _), datos in reporte.items():
        if nivel not in ['WARN', 'ERROR']:  # Solo WARN y ERROR
            continue
        ...
        try:
            if not Logger.existe_error_en_bd(datos['mensaje_normalizado'], nivel):
                ...
                # Solo para ERROR: truncar a 2000 caracteres para OpenAI
                if nivel == 'ERROR':
                ...
                # Insertar en BD con el mensaje completo (sin truncar)
                Logger.insertar_log(
                    idEmpresa=1,
                    operador=0,
                    mensaje=datos['mensaje_normalizado'],  # <- Mensaje completo
                    nivel=nivel,
                    ...
                    respuestaOpenai=respuesta_openai  # <- Respuesta de OpenAI (si aplica)
                )
                total_insertados += 1
...
def procesar_bloque(bloque_actual, linea_inicio, reporte):
    ...
    # Mantener el mensaje completo (sin truncar) para la BD
    mensaje_normalizado = mensaje_normalizado  # <- No aplicar limitar_longitud aquí
    ...
    reporte[clave].update({
        ...
        'mensaje_normalizado': mensaje_normalizado  # <- Mensaje completo
    })
...
if __name__ == "__main__":
    ...
    ARCHIVO_LOG = "C:/Users/klever.robalino/Desktop/Proyecto-Mutualista/backend-asistente/agentes/logs/logs_files/prueba15.txt"
