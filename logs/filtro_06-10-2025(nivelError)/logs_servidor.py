        ...
        with flask_app.app_context():
            ...
            return {
                ...
                "resultados": [{
                    ...
                    "nivelError": log.nivelError if log.nivelError else None
                } for log in logs]
            }
    ...
    try:
        with flask_app.app_context():
            ...
            for proceso in procesos:
                # Consulta original para niveles FATAL, ERROR, WARN
                ...
                # NUEVA CONSULTA: Métricas de criticidad por nivelError
                logs_por_criticidad = db.session.query(
                    loLogs.nivelError,
                    db.func.count(loLogs.idLogAplicacion)
                ).filter(
                    loLogs.idAuditoria == proceso.idAuditoria,
                    loLogs.nivelError.isnot(None)  # Solo logs con nivelError definido
                ).group_by(loLogs.nivelError).all()

                # Calcular métricas de criticidad
                total_errores_con_nivel = sum(count for _, count in logs_por_criticidad)
                metricas_criticidad = {
                    "criticos": 0,
                    "normales": 0,
                    "leves": 0,
                    "total_errores": total_errores_con_nivel,
                ...
                for nivel_error, count in logs_por_criticidad:
                    if nivel_error == "crítico":
                        metricas_criticidad["criticos"] = count
                    elif nivel_error == "normal":
                        metricas_criticidad["normales"] = count
                    elif nivel_error == "leve":
                        metricas_criticidad["leves"] = count

                # Calcular porcentajes
                if total_errores_con_nivel > 0:
                    metricas_criticidad["porcentaje_critico"] = round((metricas_criticidad["criticos"] / total_errores_con_nivel) * 100, 2)
                    metricas_criticidad["porcentaje_normal"] = round((metricas_criticidad["normales"] / total_errores_con_nivel) * 100, 2)
                    metricas_criticidad["porcentaje_leve"] = round((metricas_criticidad["leves"] / total_errores_con_nivel) * 100, 2)

                # Determinar nivel de alerta general
                if metricas_criticidad["porcentaje_critico"] >= 80:
                    nivel_alerta = "critico"
                elif metricas_criticidad["porcentaje_critico"] >= 30:
                    nivel_alerta = "medio"
                else:
                    nivel_alerta = "bajo"
