...
# Endpoint para actualizar configuración (con validación)
@app.post("/proceso-config/")
async def actualizar_config(config: ProcesoConfig):
    if not config.archivo:
        raise HTTPException(status_code=400, detail="Se requiere un archivo")
    proceso_config.update(config.dict())
    return {"status": "ok"}

@app.post("/procesar-log/")
async def procesar_log(nombre_archivo: str = Form(...)):
    # Ruta fija donde el servidor espera los logs (ajusta según tu entorno)
    RUTA_BASE_LOGS = "C:/Users/klever.robalino/Downloads/"
    ruta_completa = os.path.join(RUTA_BASE_LOGS, nombre_archivo)
    
    if not os.path.exists(ruta_completa):
        raise HTTPException(status_code=404, detail=f"Archivo {nombre_archivo} no encontrado en el servidor")

    try:
        bloque = ProcesosLogger.reservar_bloque(
            ruta_archivo=ruta_completa,  # Usamos la ruta fija directa
            idEmpresa=1,
            operador=0,
            bloque_size=1048576
        )
        ...
        if not bloque:
            ultimo_byte = ProcesosLogger.obtener_ultimo_byte_procesado(ruta_completa)
            print(f"ℹ️ No hay logs nuevos. Último byte procesado: {ultimo_byte}")
            return JSONResponse(
                status_code=200,
                content={
                    "status": "info",
                    "total_logs": 0,
                    "bytes_processed": f"{ultimo_byte}",
                    "details": "No hay logs nuevos para procesar"
                }
            )
        ...
        if bloque['byte_inicio'] > 0:
            with open(ruta_completa, 'rb') as f:
                f.seek(0)
                contenido_previo = f.read(bloque['byte_inicio']).decode('utf-8', errors='ignore')
                lineas_previas = contenido_previo.count('\n')
