...
async def enviar_mensaje_websocket(id_servidor: int, id_empresa: int) -> bool:
    """Envía mensaje WebSocket CON FORMATO JSON (versión corregida)."""
    max_retries = 3
    retry_delay = 0.5
    
    for attempt in range(max_retries):
        try:
            # 🔥 Cambio clave: Usar send_json_message en lugar de send_personal_message
    ...
    try:
        with flask_app.app_context():
            ...
            if servidor.idEmpresa in manager.active_connections:
                # Notificación original (se mantiene igual)
                success = loop.run_until_complete(
                    enviar_mensaje_websocket(id_servidor, servidor.idEmpresa)
                )
                
                # NUEVO: Actualización dashboard (SIN AWAIT)
    ...
    try:
        with flask_app.app_context():
            servidor = db.session.query(loServidores).filter_by(
                ...
                # se quitó esRemoto=True porque todos son remotos
            ).first()
