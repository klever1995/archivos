...
def eliminar_servidor(id_servidor: int = Path(..., description="ID del servidor a eliminar")):
    with flask_app.app_context():
        try:
            ...
            # --- PASO 1: Eliminar datos dependientes (PARA TODOS LOS SERVIDORES, NO SOLO REMOTOS) ---
            # A. Borrar logs (tanto normales como remotos)
            db.session.query(loLogs)\
                .filter_by(idServidor=id_servidor)\
                .delete()

            db.session.query(loLogsremotos)\
                .filter_by(idServidor=id_servidor)\
                .delete()

            # B. Borrar interpretaciones remotas (si existen)
            db.session.query(loInterpretacionremota)\
                .filter_by(idServidor=id_servidor)\
                .delete()

            # C. Borrar procesos vinculados al servidor (LOCALES Y REMOTOS)
            # Primero eliminamos las referencias en lo_logs que apuntan a procesos de este servidor
            procesos_del_servidor = db.session.query(LoProcesos)\
                .filter_by(idServidor=id_servidor)\
                .all()
            ...
            if ids_procesos:
                db.session.query(loLogs)\
                    .filter(loLogs.idAuditoria.in_(ids_procesos))\
                    .delete(synchronize_session=False)

                db.session.query(loLogsremotos)\
                    .filter(loLogsremotos.idAuditoria.in_(ids_procesos))\
                    .delete(synchronize_session=False)

                # Finalmente borramos los procesos
                db.session.query(LoProcesos)\
                    .filter_by(idServidor=id_servidor)\
                    .delete()

            # --- PASO OPCIONAL: Limpiar acceso remoto si ya no se usa ---
            if servidor.esRemoto and servidor.idAccesoRemoto:
                ...
                if otros_servidores == 0:
                    ...
                    # Opcional: Aquí podrías borrar también el registro de acceso remoto si lo deseas
            ...
            return {
                ...
                "tipo": "REMOTO" if servidor.esRemoto else "LOCAL"
            }
