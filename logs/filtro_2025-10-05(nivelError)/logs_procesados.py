...
# Router de FastAPI para los logs
router = APIRouter(
...
#Endpoint que devuleve los logs
@router.get("/")
def consultar_logs_aplicacion(
    ...
    idAuditoria: Optional[int] = Query(None, description="Filtrar por ID de proceso (auditoría)"),  
    ...
    try:
        with flask_app.app_context():
            ...
            if idAuditoria:
                query = query.filter(loLogs.idAuditoria == idAuditoria) 
            ...
            return {
                ...
                "resultados": [{
                    ...
                    "nivel_error": log.nivelError if log.nivelError else None  
                } for log in logs]
            }
