...
def interpretar_logs_remotos(id_servidor: int, batch_size: int = 100) -> bool:
    ...
    with app.app_context():
        try:
            ...
            # 7. Guardar en loLogs (ÚNICO CAMBIO REALIZADO)
            logs_insertar = []
            for item in logs_a_guardar:
                ...
                # CAMBIO: Ahora incluye WARN aunque no tengan respuesta
                if respuesta or item['log'].nivel == 'WARN':
                    logs_insertar.append(loLogs(
                        ...
                        respuestaOpenai=respuesta if item['log'].nivel in {'ERROR', 'FATAL'} else None,  # WARN sin respuesta
