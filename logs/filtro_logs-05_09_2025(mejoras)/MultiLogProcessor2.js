...
const toggleProcesamiento = async (idServidor, ruta, intervalo) => {
  setLoadingStates(prev => ({ ...prev, toggle: { ...prev.toggle, [idServidor]: true } }));
  ...
  try {
    ...
    console.error('Error en toggleProcesamiento:', error);
    setError(`Error: ${error.message}`);
  } finally {
    setLoadingStates(prev => ({ ...prev, toggle: { ...prev.toggle, [idServidor]: false } }));
  }
};

// Agrega nuevo servidor a monitorear
const agregarServidor = async () => {
  // Primero validar TODOS los campos ANTES de setear el estado de carga
  ...
  setLoadingStates(prev => ({ ...prev, agregar: true }));
  ...
  try {
    ...
    const filtroActual = departamentoFiltro;
    await cargarDatosIniciales();
    setDepartamentoFiltro(filtroActual);
    ...
    setLoadingStates(prev => ({ ...prev, agregar: false }));
  }
};

// Elimina servidor y sus procesos asociados
  const eliminarServidor = async (idServidor) => {
    setLoadingStates(prev => ({ ...prev, eliminar: { ...prev.eliminar, [idServidor]: true } }));
    ...
    try {
      ...
      setLoadingStates(prev => ({ ...prev, eliminar: { ...prev.eliminar, [idServidor]: false } }));
    }
  };
  ...
  return (
    <div className="multi-log-container">
      ...
      <div className="agregar-archivo">
        ...
        />
        <button onClick={agregarServidor} disabled={loadingStates.agregar}>
          {loadingStates.agregar ? '⏳ Agregando...' : 'Agregar Servidor'}
        </button>
      </div>
                ...
                return (
                  <tr key={s.idServidor}>
                    ...
                    <button
                        ...
                        disabled={loadingStates.toggle[s.idServidor]}
                      >
                        {loadingStates.toggle[s.idServidor] ? '⏳' : estaActivo ? 'Detener' : 'Iniciar'}
                        </button>
                        <button
                          onClick={() => eliminarServidor(s.idServidor)}
                          className="eliminar"
                          disabled={loadingStates.eliminar[s.idServidor]}
                        >
                          {loadingStates.eliminar[s.idServidor] ? '⏳ Eliminando' : 'Eliminar'}
                        </button>
                    </td>
                  </tr>
                );
