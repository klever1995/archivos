...
def procesar_log_en_segundo_plano(nombre_archivo: str, idServidor: int, bloque_size: Optional[int] = None):
    ...
    try:
        with flask_app.app_context():
            ...
            # Modificación 1: Verificar si el archivo es pequeño
            tamaño_archivo = os.path.getsize(ruta_completa)
            es_archivo_pequeno = tamaño_archivo < 1024  # Consideramos pequeño si es menor a 1KB
            
            if es_archivo_pequeno:
                # Procesamiento especial para archivos pequeños
                logger.info(f"🔹 Archivo pequeño detectado ({tamaño_archivo} bytes). Procesando completo...")
                
                with open(ruta_completa, 'r', encoding='utf-8', errors='ignore') as f:
                    contenido_completo = f.read()
                
                # Crear un bloque artificial para el archivo completo
                bloque = ProcesosLogger.reservar_bloque(
                    ruta_archivo=ruta_completa,
                    idEmpresa=1,
                    operador=0,
                    idServidor=idServidor,
                    bloque_size=tamaño_archivo,
                    forzar_completo=True  # Nuevo parámetro para indicar procesamiento completo
                )
                
                bloques_procesados = extraer_bloques_log(contenido_completo, offset_linea=0)
            else:
                # Procesamiento normal para archivos grandes
                bloque_size_recomendado = min(tamaño_archivo // 10, 10485760)
            ...
            # Procesar los bloques obtenidos (tanto para archivos pequeños como grandes)
            if bloques_procesados:
                ...
                ProcesosLogger.finalizar_proceso(
                    ...
                    ultimo_byte=bloque['byte_fin'] if not es_archivo_pequeno else tamaño_archivo
                )

                logger.info(f"Procesamiento finalizado. Total logs insertados: {total_logs}")
            else:
                logger.info("No se encontraron bloques de log válidos para procesar")
