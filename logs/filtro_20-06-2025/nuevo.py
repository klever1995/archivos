...
def extraer_bloques_log(chunk: str, offset_linea: int = 0) -> list:
    ...
    if bloque_actual and extraer_nivel(bloque_actual[0]) not in ['INFO', 'DEBUG']:
        bloques.append({
            'linea_inicio': linea_inicio,
            'contenido': ''.join(bloque_actual)
        })
...
def generar_reporte_logs(bloques: list, idServidor: int, idAuditoria: int) -> dict:
    ...
    for i, bloque in enumerate(bloques, start=1):
        nivel = extraer_nivel(bloque['contenido'])
...
def procesar_log_en_segundo_plano(nombre_archivo: str, idServidor: int, bloque_size: Optional[int] = None):
    ...
    if nombre_archivo in procesos_activos_por_archivo:
        logger.info(f"🔄 El archivo {nombre_archivo} ya está siendo procesado")
        return
    ...
    try:
        with flask_app.app_context():
            ...
            if not servidor:
                logger.error("❌ Servidor no encontrado")
                return
            ...
            ultimo_byte = ProcesosLogger.obtener_ultimo_byte_procesado(ruta_completa, idServidor)

            logger.info(f"📊 Tamaño archivo: {tamaño_actual} bytes | Último byte: {ultimo_byte}")

            # 2. Registrar nuevo proceso (SIEMPRE se crea, incluso si no hay cambios)
            nuevo_proceso = LoProcesos(
                idEmpresa=1,
                idServidor=idServidor,
                operador=0,
                byte_inicio=ultimo_byte,
                byte_fin=tamaño_actual,
                ultimo_byte_procesado=ultimo_byte,
                totalLogsProcesados=0,  # Valor por defecto
                estado='EN_PROCESO',
                archivo=ruta_completa,
                fechaInicio=datetime.now(),
                bloque_size=bloque_size or min(tamaño_actual // 5, 25485760)
            )
            db.session.add(nuevo_proceso)
            db.session.commit()
            idAuditoria = nuevo_proceso.idAuditoria

            # 3. Si el archivo no ha crecido
            if tamaño_actual <= ultimo_byte:
                logger.info("🟢 Archivo sin cambios. Actualizando registro...")
                nuevo_proceso.estado = 'COMPLETADO'
                nuevo_proceso.fechaFin = datetime.now()
                db.session.commit()
                return
            ...
            try:
                with open(ruta_completa, 'rb') as f:
                    f.seek(ultimo_byte)
                    contenido = f.read(tamaño_actual - ultimo_byte).decode('utf-8', errors='ignore')

                bloques = extraer_bloques_log(contenido, offset_linea=0)
                bloques_importantes = [b for b in bloques if extraer_nivel(b['contenido']) in ['WARN', 'ERROR', 'FATAL']]

                if bloques_importantes:
                    reporte = generar_reporte_logs(bloques_importantes, idServidor, idAuditoria)
                    total_logs = sum(datos['count'] for datos in reporte.values())
                    nuevo_proceso.totalLogsProcesados = total_logs
                    logger.info(f"✅ Encontrados {total_logs} logs importantes")
                else:
                    logger.info("🔍 No hay logs importantes en el nuevo contenido")

            except Exception as e:
                logger.error(f"❌ Error procesando contenido: {str(e)}")
                nuevo_proceso.estado = 'ERROR'
                raise

            # 5. Finalizar proceso
            nuevo_proceso.estado = 'COMPLETADO'
            nuevo_proceso.fechaFin = datetime.now()
            nuevo_proceso.ultimo_byte_procesado = tamaño_actual
            db.session.commit()
        ...
        if idAuditoria:
            db.session.rollback()
            with flask_app.app_context():
                proceso = db.session.get(LoProcesos, idAuditoria)
                if proceso:
