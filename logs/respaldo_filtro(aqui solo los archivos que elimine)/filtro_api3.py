...
def extraer_bloques_log(chunk: str, offset_linea: int = 0) -> list:
    ...
    bloque_actual = []
    linea_inicio = None
    lineas = chunk.splitlines(keepends=True)  # splitlines es eficiente para chunks de 10MB

    for i, linea in enumerate(lineas, start=offset_linea):
        if es_inicio_log(linea):
            if bloque_actual:  # Guardar bloque anterior si existe
                bloques.append({
                    'linea_inicio': linea_inicio,
                    'contenido': ''.join(bloque_actual)  # join es más eficiente que += para strings
                })
            bloque_actual = [linea]
            linea_inicio = i
        elif bloque_actual:  # equivalente a "en_bloque = True"
            if linea.startswith(("   ", "\t", "at ")):  # Patrones directos sin regex
    ...
    # Añadir último bloque si queda pendiente
    if bloque_actual:
        bloques.append({
            'linea_inicio': linea_inicio,
            'contenido': ''.join(bloque_actual)
...
def insertar_logs_a_bd(reporte: dict, idServidor: int, idAuditoria: int) -> int:
    ...
    logs_nuevos = []
    logs_a_insertar = []
    fecha_actual = datetime.now()  # Obtenemos la fecha actual una sola vez

    try:
        with flask_app.app_context():
            # 1. Preparar datos para batch
            for i, ((nivel, categoria, _), datos) in enumerate(reporte.items(), start=1):
                ...
                logs_a_insertar.append({
                    ...
                    'fechaCreacion': fecha_actual  # Añadimos el campo requerido
                })
            ...
            # 2. Procesar OpenAI en paralelo (solo para logs nuevos)
            if logs_nuevos:
                ...
                # Actualizar respuestas en logs_a_insertar
                ...
                # Batch insert para errores nuevos (también añadiendo fechaCreacion)
                db.session.bulk_insert_mappings(loErrorconocido, [
                    {
                        ...
                        'fechaCreacion': fecha_actual  # Añadir también para loErrorconocido si es necesario
                    } for log, respuesta in zip(logs_nuevos, respuestas)
                ])

            # 3. Batch insert principal
...
def procesar_log_en_segundo_plano(nombre_archivo: str, idServidor: int, bloque_size: Optional[int] = None):
    ...
    try:
        with flask_app.app_context():
            ...
            bloque_size_recomendado = min(tamaño_archivo // 5, 25485760)
            ...
            if not bloque:
                logger.info("No hay logs nuevos para procesar")
                return
