...
const ModalAccesos = ({ idEmpresa, onClose }) => {
    ...
    try {
      const response = await fetch(`http://localhost:8000/api/accesos-remotos/${accesoActual.idAcceso}`, {
        ...
        body: JSON.stringify({
          ...
          contrasena: accesoActual.contrasena || undefined // Solo enviar si hay cambio
        }),
      });
  ...
  const handleEditar = (acceso) => {
    setAccesoActual({
      ...
      contrasena: '', // Dejar vacío por seguridad
  ...
  // ... (resto del código de loading y error permanece igual)
