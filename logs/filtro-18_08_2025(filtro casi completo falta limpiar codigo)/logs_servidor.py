...
class ServidorResponse(BaseModel):
    ...
    fecha_registro: str
    total_procesos: int
    id_acceso_remoto: Optional[int] = None  
...
def consultar_servidores(
    ...
    idDepartamento: Optional[int] = Query(None, description="ID del departamento para filtrar"),  # Nuevo parámetro
    ...
    try:
        with flask_app.app_context():
            ...
            # Obtener nombres de departamentos para la respuesta
            ...
            return {
                "meta": {
                    ...
                    "parametros": {
                        "idEmpresa": idEmpresa,
                        "idDepartamento": idDepartamento,  # Nuevo parámetro
                ...
                "resultados": [{
                    ...
                    "id_departamento": s.idDepartamento,  # Nuevo campo
                    "nombre_departamento": departamentos.get(s.idDepartamento)  # Nuevo campo
                } for s in servidores]
            }
...
def crear_servidor(servidor_data: ServidorCreate):
    try:
        with flask_app.app_context():
            # Validar servidor duplicado (existente)
            ...
            # Validar que idAccesoRemoto exista (existente)
            ...
            # Validar que idDepartamento exista (si se proporciona)  # Nuevo
            if servidor_data.idDepartamento:
                departamento = db.session.query(loDepartamentos).filter_by(
                    idDepartamento=servidor_data.idDepartamento
                ).first()
                if not departamento:
                    raise HTTPException(
                        status_code=404,
                        detail=f"No existe el departamento con ID {servidor_data.idDepartamento}"
                    )

            # Crear servidor (añadir idDepartamento)  # Nuevo
            nuevo_servidor = loServidores(
                ...
                idDepartamento=servidor_data.idDepartamento  # Nuevo campo
            )
